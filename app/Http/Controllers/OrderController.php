...
class OrderController extends Controller
{
    ...
    public function event_placeOrder(Request $request){
        ...
        $data = [
            ...
            'event_place'=>$request->address_line_1,
            'event_price'=>$eventDet->event_price,
            'email'     =>$request->email,
            'age'   => $request->age,
            'medical_condition'=>$request->medical_condition,
            'emergency_contact_name'=> $request->emergency_contact_name,
         ...
         $mail_data=array('order_id'=>$order_id,
                          'site_name'=>$site_name,
                          'customer_name' =>  $CustomerDet->fname." ".$CustomerDet->lname,
            'event_name'=>$EventDet->event_name,
            'event_date'=>$EventDet->event_date,
        ...
        Mail::to($email)->send(new SendAdminMail($mail_data)); 
        Mail::to($request->email)->send(new SendCustomerMail($mail_data)); 
        
        
        
    /*    Mail::send('mail.event-booking',$data, function($message) use($email, $site_name) {
            $message->from(env("MAIL_FROM_ADDRESS"),$site_name);
            $message->to($email)->subject('New Event Booking Notification - Cricademia');
        }); */
            ...
            $session = \Stripe\Checkout\Session::create([
                ...
                'cancel_url'  => route('event-thank-you'),
            ]);
